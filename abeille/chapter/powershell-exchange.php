<!DOCTYPE html>
<html lang="fr">
<head>
        <link rel="apple-touch-icon" sizes="180x180" href="../img/logo-abeille.jpg">
        <link rel="icon" type="image/png" sizes="32x32" href="../img/logo-abeille.jpg">
        <link rel="icon" type="image/png" sizes="16x16" href="../img/logo-abeille.jpg">
        <!-- og -->
        <meta property="og:url" content="undefined">
        <meta property="og:title" content="Website - Le rapport de stage chez Abeille">
        <meta property="og:image" content="../img/logo-abeille.jpg">
        <meta property="og:description" content="Website - Le rapport de stage">
        <meta property="og:type" content="website">
        <meta property="og:site_name" content="Website - Le rapport de stage chez Abeille">
        <meta name="description" content="Website - Le rapport de stage chez Abeille">
        <!-- endsection -->
        <meta charset="UTF-8">  
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Introduction - Website</title>
        <link rel="stylesheet" href="../css/reset.css">
        <link rel="stylesheet" href="../css/main.css">
</head>
<body>
        
        <div class="ui-controller">
                <?php require '../components/menu-bar.html'; ?>
                <div id="ui-standard" class="ui-standard">
                        <div class="center">
                                <p class="title">PShell Snippets Exchange</p>

                                <p class="content">WifiMax est une technologie développée par Abeille qui est comprise dans l'offre de téléphonie via internet BeeIp.
                                        C'est un forfait qui comprend un accès internet entre vingt et cent mégabits par secondes pour téléphoner et obtenir des chaînes
                                        de télévision en ligne. L'argument principal de vente est que c'est la seule alternative à la fibre dans les campagnes. L'abonnement
                                        revient tout de même à trente-neuf euros par mois. Cette offre est sans engagement et sans frais de résiliation.
                                </p>
                                <br>
                                <figure>
                                        <img src="https://cdn.guidingtech.com/media/assets/WindowsTerminal_Cover.png">
                                        <figcaption>Image de présentation de Windows Terminal</figcaption>
                                </figure>
                                <br><br>
                        </div>
                </div>
        </div>

        <script src="../script.js"></script>
</body>
</html>